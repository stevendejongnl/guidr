...
- **This is the exhaustive list** — see `.releaserc.json`'s `commit-analyzer` `releaseRules`. Any other type (`diag:`, `wip:`, a typo, anything not above) is **silently treated as no release** by semantic-release: the `Release` workflow runs, exits green, and nothing gets built or shipped — no error, no warning. Confirm the merged commit's type is in this list whenever a change is meant to ship; if it's already merged with the wrong type, fix it with a new, correctly-typed commit rather than rewriting the merged commit — see the empty-commit caveat below before using `--allow-empty` for this.
- **Rules**: No AI attribution, rebase before push, **ALWAYS validate pre-push before pushing**
- **Empty commits + "Rebase and merge" don't mix**: this repo's PRs merge via GitHub's "Rebase and merge" (squash is disabled repo-wide; see `allow_squash_merge` in repo settings), which — like plain `git rebase` — silently drops any commit whose diff is empty. An empty `fix:` commit used to trigger a stranded release will vanish on merge with no error, and semantic-release will just report "No release needed" again. If you need to trigger a release for already-merged-but-unshipped code, either (a) merge that PR with **"Create a merge commit"** instead (2-parent commit, nothing gets rebased away — `gh pr merge <n> --merge`), or (b) make the triggering commit carry a real, non-empty diff so it survives rebase either way.
