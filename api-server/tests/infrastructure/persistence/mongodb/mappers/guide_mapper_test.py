...
class TestGuideMapper:
    ...
    def test_map_guide_to_document(self) -> None:
        ...
        assert document["categoryId"] == category_id.value
    ...
    def test_map_document_to_guide(self) -> None:
        ...
        assert guide.category_id == EntityId(category_id)
    ...
    def test_map_document_without_optional_fields(self) -> None:
        """Test mapping a document without optional fields (backward compatibility)."""
        guide_id = str(uuid4())
        category_id = str(uuid4())
        now = datetime.now(UTC)

        # Legacy document without new fields
        document = {
            "_id": guide_id,
            "categoryId": category_id,
            "title": "Learn Python",
