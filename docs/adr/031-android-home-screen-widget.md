...
- **Self-correcting completion**: `Chronometer` has no built-in stop-at-zero — left alone it just keeps ticking into negative time forever. The app previously only flipped the widget to "Complete" when `GuideDetailScreen`'s live completion-detection effect noticed, which requires the JS thread to be running and that screen mounted — if the app was backgrounded when a timer finished, the widget would count into the negatives until the app was reopened to that screen. `saveState()` now schedules a one-shot `AlarmManager` alarm (`ACTION_TIMER_COMPLETE_CHECK`) for the exact completion moment whenever a timer is running; `onReceive` flips the stored state to complete (if it's still the same running timer — not superseded by a pause/reset/new-timer in the meantime) and refreshes the widget, independent of whether the app is running at all.
